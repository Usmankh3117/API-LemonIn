...
var timer = 0;
var AppStateOnOff = false;
var LinkOpenerWindowType = "NewTab";
var limUserId = "";
...
function CheckAppState() {
    chrome.storage.local.get(['ApppState', "UserSetting", "LemonInUser"], function (result) {
        ...
        if (result != undefined && result.UserSetting != undefined) {
            LinkOpenerWindowType = result.UserSetting.LinkOpener;
        }
        limUserId = result.LemonInUser.Id;
    });
}
...
chrome.extension.sendMessage({
    ...
    if (response === null) {
        console.log("Unable to load linkopner due to null response");
    } else {
        if (response.hasOwnProperty("error")) {
            console.log("Unable to properly load linkopner, returning to default settings: " + JSON.stringify(response));
        }
        ...
        for (var i in response.blocked) {
            ...
            if (re.test(window.location.href)) {
                allowed = false;
                console.log("linkopner is blocked on this site: " + response.blocked[i] + "~" + window.location.href);
            }
        }
    ...
    if (allow_selection()) {
        ...
        if (timer !== 0) {
            ...
            if (this.box === null) {
                ...
                this.box.style.border = "2px solid" + this.settings[this.setting].color;
...
function detech(x, y, open) {
    ...
    for (var i = 0; i < this.links.length; i++) {
        if ((!this.smart_select || this.links[i].important) && !(this.links[i].x1 > this.box.x2 || this.links[i].x2 < this.box.x1 || this.links[i].y1 > this.box.y2 || this.links[i].y2 < this.box.y1)) {
            if (open) {
                open_tabs.push({
                    "url": this.links[i].href != undefined && this.links[i].href.endsWith("/") == true ? this.links[i].href.slice(0, this.links[i].href.length - 1) : this.links[i].href,
            ...
            if (this.links[i].box === null) {
                ...
                link_box.style.id = "Limlink-link";
    ...
    if (open_tabs.length > 0) {
        ...
        GetLastDaysLinks(limUserId, open_tabs);
        //DeductAccountCredit(limUserId, open_tabs.length);
        chrome.extension.sendMessage({
            message: "activate",
            linkOpenerType: LinkOpenerWindowType,
            urls: open_tabs,
...
function allow_selection() {
    for (var i in this.settings) {
        ...
        if (this.settings[i].mouse == this.mouse_button && this.settings[i].key == this.key_pressed && AppStateOnOff == true) {
            this.setting = i;
            if (this.box !== null) {
                this.box.style.border = "2px solid " + this.settings[i].color;
            }
            return true;
        }
    }
                ...
                if (data.status) {
                    ...
                    if (lastDayLinks != undefined && lastDayLinks.length > 0) {
                        ...
                        for (var i = 0; i < currentLinks.length; i++) {
                            if (lastDayLinks.indexOf(currentLinks[i].url) == -1)
                                credit++;
                        }
