...
function openURLinNewWindow(urls, delay, windowId, tabPosition, closeTime) {

    if (windowId == 0) {
        chrome.windows.create({
            url: urls.shift().url
        }, function (win) {
            let newwindowId = win.id;
            window.setTimeout(function () { openURLinNewWindow(urls, delay, newwindowId, tabPosition, closeTime) }, delay * 1000);
        });
    }
    else {
        var obj = {
        ...
        if (urls.length > 0) {
            window.setTimeout(function () { openURLinNewWindow(urls, delay, windowId, tabPosition, closeTime) }, delay * 1000);
        }
    }
}

function openURLinincognitoWindow(urls, delay, windowId, tabPosition, closeTime) {
    ...
    if (windowId == 0) {
        chrome.windows.create({
            ...
            let newwindowId = win.id;
            window.setTimeout(function () { openURLinincognitoWindow(urls, delay, newwindowId, tabPosition, closeTime) }, delay * 1000);
        });
    }
    else {
        var obj = {
        ...
        if (urls.length > 0) {
            window.setTimeout(function () { openURLinincognitoWindow(urls, delay, windowId, tabPosition, closeTime) }, delay * 1000);
        }
    }
}
        ...
        case "activate":
            ...
            switch (request.setting.action) {
                ...
                case "bm":
                    chrome.bookmarks.getTree(
                        function (bookmarkTreeNodes) {
                            ...
                            chrome.bookmarks.create({ "parentId": bookmarkTreeNodes[0].children[1].id, "title": "Lemlinks " + timeConverter(new Date()) },
                                function (newFolder) {
                ...
                case "win":
                    ...
                    break;
                case "tabs":
                    if (request.linkOpenerType == "NewTab") {
                    ...
                    else if (request.linkOpenerType == "NewWindow") {
                        openURLinNewWindow(request.urls, request.setting.options.delay, 0, 0, request.setting.options.close);
                    }
                    else if (request.linkOpenerType == "InognitoWindow") {
                        openURLinincognitoWindow(request.urls, request.setting.options.delay, 0, 0, request.setting.options.close);
                    }
...
if (!settingsManager.isInit()) {
    ...
    // inject into windows currently open to make it just work
    chrome.windows.getAll({ populate: true }, function (windows) {
        for (var i = 0; i < windows.length; ++i) {
            for (var j = 0; j < windows[i].tabs.length; ++j) {
                if (!/^https?:\/\//.test(windows[i].tabs[j].url)) continue;
                chrome.tabs.executeScript(windows[i].tabs[j].id, { file: "/scripts/linkOpenerMaster.js" });
            }
        }
    });
